...
const getArticles = async (): Promise<void> => {
  articles = [];
  try {
    const response: Response = await fetch(`https://restapi.fr/api/ormide?sort=${sort}`);
...
const filterEventListener = (): void => {
  ...
  sortDateNode.addEventListener("click", () => {
    sort == "createdAt:asc" ? (sort = "createdAt:desc") : (sort = "createdAt:asc");
    sort == "createdAt:asc"
...
const spinning = (bool: boolean): void => {
  ...
  bool ? (articlesNode.innerHTML = `<i class="fa-solid fa-spinner spin"></i>`) : null;
};
